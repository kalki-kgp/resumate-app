...
interface AuthenticatedThumbnailImageProps {
  thumbnailSrc: string;
  alt: string;
}

const AuthenticatedThumbnailImage = ({ thumbnailSrc, alt }: AuthenticatedThumbnailImageProps) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(thumbnailSrc, {
        ...
        // Keep the placeholder; thumbnails are non-critical UI.
      }
    };

    // Reset first so old thumbnails don't flash during thumbnailSrc changes.
    setObjectUrl(null);
    load();
  ...
  if (objectUrl) {
    return <img src={objectUrl} alt={alt} className="w-full h-full object-cover object-top" />;
  }
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 w-14 h-18 rounded-md overflow-hidden bg-[#f3f4f6] border border-[#e4e7eb]">
                      {thumbnailSrc ? (
                        <AuthenticatedThumbnailImage thumbnailSrc={thumbnailSrc} alt={resume.title} />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
                          <FileText size={20} className="text-[#ccd0d5]" />
                        </div>
