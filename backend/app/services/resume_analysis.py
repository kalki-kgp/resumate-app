...
You are NOT doing job-match scoring (no job description is provided). Instead, estimate how well the resume will be parsed and screened by typical ATS systems based on formatting + extractability + content signals.

Compute ats_score_estimate using the weighted criteria below (internally). Then output a single integer 0-100:
1) Formatting compatibility & extractability (0-45)
   - 45: single-column, clean headings, standard section structure, text-based content that reads well.
   - 20-44: mostly ATS-readable, but with minor barriers (e.g., inconsistent headings, light decoration, cramped layout).
   - 0-19: likely ATS parsing issues (multi-column designed for humans, text embedded in images, heavy tables/graphics, missing/unclear headings).
2) Structure & field completeness (0-25)
   - 25: all key sections are present and clearly labeled (Summary/Headline, Experience, Education, Skills/Tools).
   - 13-24: most sections present but some are missing or hard to extract.
   - 0-12: multiple missing sections or unclear/merged sections.
3) Keyword/skill signal strength (0-20)
   - 20: skills/tools are explicit and aligned with what is stated in experience; clear "Skills" section or equivalent.
   - 10-19: some skills present but incomplete or not clearly extractable.
   - 0-9: skills/tools largely absent or only implied.
4) Evidence clarity & impact density (0-10)
   - 10: outcomes/metrics/strong action verbs are visible in most experience bullets.
   - 5-9: some outcomes/metrics but inconsistent.
   - 0-4: mostly vague duties with no measurable impact visible.

Calibration / anti-clustering requirements:
- Use the full 0-100 range; do not default to ~70-80.
- If there is any strong ATS risk (multi-column layout, unreadable text, sections that appear as images/graphics), cap at 60 unless the rest of the resume is clearly extractable and well-structured.
- If you are uncertain about ATS extractability, score conservatively (bias 10-20 points lower rather than higher).
